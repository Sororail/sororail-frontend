// ── Deployment note (#495) ─────────────────────────────────────────────────────
//
// This Cloudflare Worker is an ALTERNATIVE keeper for environments where the
// compiled Rust binary (oracle/src/keeper_loop.rs) cannot be deployed (e.g.
// serverless edge). Running BOTH against the same Stellar keypair simultaneously
// causes sequence-number conflicts and double-submissions. Set KEEPER_ENABLED to
// "false" in wrangler.toml (or via a Worker secret) for whichever side should be
// inactive before going live with both binaries deployed.
...
interface Env {
  ...
  RPC_URL: string;
  /** Horizon base URL — used for balance checks. Defaults to testnet. */
  HORIZON_URL: string;
  ...
  /**
   * Minimum XLM balance the keeper must hold before submitting any transaction.
   * Mirrors MIN_KEEPER_BALANCE_XLM used by the Rust deployment (fly.toml).
   * Defaults to "1.0" if unset.
   */
  MIN_KEEPER_BALANCE_XLM?: string;
  /**
   * Set to "false" to disable keeper execution while keeping the worker live.
   * Use this when the Rust keeper_loop is already active (#495).
   */
  KEEPER_ENABLED?: string;
}

// ── Module-level concurrency guard (#488) ──────────────────────────────────────
//
// Prevents a manual /run invocation from overlapping a scheduled cron firing
// within the same Worker isolate. Workers are single-threaded per isolate but
// cron and fetch handlers CAN run concurrently in the same isolate instance.
//
// This guard covers same-isolate races. For cross-isolate protection (e.g.
// two Cloudflare edge nodes firing crons simultaneously) a KV-backed
// distributed lease should be used in production.
...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    return Response.json({
      status: "ok",
      hint: "POST /run to trigger manually",
      // Surface whether the keeper is actually enabled so operators can
      // confirm the KEEPER_ENABLED flag before assuming the cycle will run.
      keeperEnabled: (env.KEEPER_ENABLED ?? "true") !== "false",
    });
  },
};
...
async function runKeeperCycle(env: Env): Promise<object> {
  ...
  // #495: honour the kill-switch so operators can disable the TS keeper while
  // the Rust keeper_loop is active, preventing two uncoordinated signers.
  if ((env.KEEPER_ENABLED ?? "true") === "false") {
    log("Keeper disabled via KEEPER_ENABLED=false — skipping cycle");
    return { status: "disabled", logs };
  }

  // #488: reject concurrent invocations (same-isolate guard).
  if (cycleRunning) {
    log("Keeper cycle already in progress — skipping to prevent double-processing");
    return { status: "skipped_concurrent", logs };
  }

  cycleRunning = true;
  try {
    return await _runKeeperCycleInner(env, log);
  } finally {
    cycleRunning = false;
  }
}

async function _runKeeperCycleInner(
  env: Env,
  log: (msg: string) => void,
): Promise<object> {
  ...
  // #489: verify the keeper account holds enough XLM before building any
  // transaction. This mirrors the MIN_KEEPER_BALANCE_XLM guard in the Rust
  // keeper (oracle/src/keeper.rs). Without this check, every transaction
  // submitted during a low-balance cycle fails with a cryptic error.
  const horizonUrl = env.HORIZON_URL ?? "https://horizon-testnet.stellar.org";
  const minBalanceXlm = parseFloat(env.MIN_KEEPER_BALANCE_XLM ?? "1.0");
  log(`Checking keeper balance (min: ${minBalanceXlm} XLM)…`);
  const balanceXlm = await getKeeperBalanceXlm(callerAddr, horizonUrl);
  log(`Keeper balance: ${balanceXlm.toFixed(7)} XLM`);
  if (balanceXlm < minBalanceXlm) {
    const msg = `Keeper balance ${balanceXlm.toFixed(7)} XLM is below minimum ${minBalanceXlm} XLM — aborting cycle`;
    log(msg);
    return { status: "insufficient_balance", balanceXlm, minBalanceXlm, logs };
  }
...
/**
 * Fetch the keeper account's native XLM balance from Horizon.
 * Throws if the account is unfunded or the Horizon call fails.
 */
async function getKeeperBalanceXlm(callerAddr: string, horizonUrl: string): Promise<number> {
  const url = `${horizonUrl.replace(/\/$/, "")}/accounts/${callerAddr}`;
  const resp = await fetch(url);
  if (resp.status === 404) {
    throw new Error(`Keeper account ${callerAddr} not found on Horizon — account may be unfunded`);
  }
  if (!resp.ok) {
    throw new Error(`Horizon account fetch failed: ${resp.status} ${resp.statusText}`);
  }
  ...
  if (!native) {
    throw new Error("No native XLM balance entry found in Horizon account response");
  }
