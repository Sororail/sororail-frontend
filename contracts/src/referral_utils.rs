...
use crate::{
    data_store::DataStoreClient, keys::claimable_referral_amount_key,
    referral_storage::ReferralStorageClient, types::TierConfig,
};
...
pub fn compute_position_fee(size_delta_usd: u128, position_fee_factor: u128) -> u128 {
    ...
    size_delta_usd.saturating_mul(position_fee_factor) / crate::pricing_utils::FACTOR_DENOMINATOR
}
...
pub fn split_referral_fee(position_fee: u128, tier: &TierConfig) -> (u128, u128) {
    ...
    let discount =
        position_fee.saturating_mul(tier.discount_bps as u128) / REFERRAL_FACTOR_DENOMINATOR;
    let rebate = position_fee.saturating_mul(tier.rebate_bps as u128) / REFERRAL_FACTOR_DENOMINATOR;
    (discount, rebate)
}
