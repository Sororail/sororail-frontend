...
> [!IMPORTANT]
> **None of the behaviour described below is implemented in `so4-oracle`.**
>
> This document describes the risk engine of the **external SO4 perps Soroban
> contracts**, which live in a separate repository. Every module and function
> referenced here — `position_handler::is_liquidatable`,
...
> the insurance fund — exists only in those contracts. A repo-wide search of
> `so4-oracle` finds none of them.
>
> The only `order_handler` in this repository is the `order_handler_contract_id`
> config value: an opaque address pointing at one of those externally-deployed
> contracts. The keeper loop in [`oracle/src/keeper_loop.rs`](../oracle/src/keeper_loop.rs)
> invokes exactly five contract entrypoints — `set_prices`, `execute_order`,
> `freeze_order`, `execute_deposit`, `execute_withdrawal` — and never queries
> liquidation or ADL state. **This service performs no liquidation monitoring,
> no ADL execution, and no insurance-fund accounting.**
>
> The `PRECISION = 1_000_000` constant used for the margin math below is the
> contracts' margin precision. It is unrelated to this repo's price scaling
> constant (`FLOAT_PRECISION = 1e30`, see [`oracle/src/binance.rs`](../oracle/src/binance.rs)).
>
> Keep this file as background reading for the contract system the keeper
> submits to; do not read it as a specification of `so4-oracle` behaviour.
