...
async fn test_request_id_and_completion_logs() {
    ...
    let generated_id = res1.headers().get("x-request-id").expect("should generate request id");
    ...
    let echoed_id = res2.headers().get("x-request-id").expect("should echo request id");
    ...
    let error_id = res3.headers().get("x-request-id").expect("should echo request id on error");
    assert_eq!(error_id, "error-id-123", "request id should be echoed on error");
}
...
async fn test_unmatched_route_metrics_cardinality() {
    ...
    assert!(metrics_out.contains("oracle_http_requests_total{route=\"/unmatched\",method=\"GET\",status_class=\"4xx\"} 10"));
...
async fn test_promtool_validation() {
        ...
        .spawn()
    {
        ...
        std::thread::spawn(move || {
            stdin.write_all(metrics_out.as_bytes()).expect("Failed to write to stdin");
        });
        ...
        if !output.status.success() {
            ...
            panic!("promtool check metrics failed!\nSTDOUT:\n{}\nSTDERR:\n{}", stdout, stderr);
        }
    } else {
