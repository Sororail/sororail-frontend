...
use std::sync::Arc;
use apis::state::{AppState, Reader, ReaderError, MarketSummary};
...
impl Reader for MockReader {
    ...
    async fn get_position_info(&self, position_id: &str) -> Result<serde_json::Value, ReaderError> {
        if position_id == "p1" {
            Ok(serde_json::json!({"id":"p1","size":10.0,"collateral":100.0,"entry_price":1.0,"index_token":"gbpaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","liquidation_price":0.5,"pending_fees":1.0}))
        } else {
            Ok(serde_json::json!({}))
        }
    }
...
async fn test_prices_token_found_case_insensitive() {
    ...
    let state = AppState { cache: Cache::new(), reader: Arc::new(MockReader) };
    let app = Router::new()
        .route("/prices/:token", axum::routing::get(apis::server::get_price))
...
async fn test_markets_list_cached_and_populated() {
    ...
    let app = Router::new()
        ...
        .layer(axum::Extension(Arc::new(state)));
    let req = Request::builder().uri("/markets").body(Body::empty()).unwrap();
    let resp = app.clone().oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    // second call should be served from cache; still OK
    let req2 = Request::builder().uri("/markets").body(Body::empty()).unwrap();
...
async fn test_market_detail_and_positions() {
    let state = AppState { cache: Cache::new(), reader: Arc::new(MockReader) };
    let app = Router::new()
        .route("/markets/:market_id", axum::routing::get(apis::server::get_market))
        .layer(axum::Extension(Arc::new(state)));
    let req = Request::builder().uri("/markets/m1").body(Body::empty()).unwrap();
...
async fn test_positions_empty_returns_empty_array() {
    ...
    impl Reader for EmptyReader {
        async fn get_markets(&self) -> Result<Vec<String>, ReaderError> { Ok(vec![]) }
        async fn get_market_pool_value_info(&self, _market: &str) -> Result<MarketSummary, ReaderError> { Err(ReaderError::RpcError) }
        async fn get_market_detail(&self, _market: &str) -> Result<serde_json::Value, ReaderError> { Err(ReaderError::RpcError) }
        async fn get_account_positions(&self, _account: &str) -> Result<Vec<String>, ReaderError> { Ok(vec![]) }
        async fn get_position_info(&self, _position_id: &str) -> Result<serde_json::Value, ReaderError> { Err(ReaderError::RpcError) }
        async fn get_latest_price(&self, _token: &str) -> Result<f64, ReaderError> { Ok(0.0) }
    }
    let state = AppState { cache: Cache::new(), reader: Arc::new(EmptyReader) };
    let app = Router::new()
        .route("/positions/:account", axum::routing::get(apis::server::get_positions))
        .layer(axum::Extension(Arc::new(state)));
    let req = Request::builder().uri("/positions/g0000000000000000000000000000000000000000000000000000").body(Body::empty()).unwrap();
    let resp = app.oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
